...
for m in range(M):
  ...
  #   number = np.random.randint(low=0,high=len(X_train))
  #   X_data.append(X_train[number])
  #   y_data.append(y_train[number])

  indexs = np.random.randint(0, len(X_train), len(X_train))
  X_data = X_train[indexs]
  y_data = y_train[indexs]
