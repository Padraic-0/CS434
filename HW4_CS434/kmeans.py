...
def imageProblem():
  ...
  k=5
  centroids, assignments, SSE = kMeansClustering(img_feats, k, 30, min_size=5)
  print(SSE[-1])
  # Visualize Clusters
  print("Assignments:", assignments)
  print("Unique clusters in assignments:", np.unique(assignments))
...
def initalizeCentroids(dataset, k):
  n, d = dataset.shape
  centroids_index = np.random.choice(n,size=k, replace=False)
  centroids = dataset[centroids_index]
  return centroids
...
def computeAssignments(dataset, centroids):
  ...
  for centroid in centroids:
    centroids_assigment.append(np.linalg.norm(centroid - dataset, axis = 1))


  assignments = [0] * dataset.shape[0]
  for i,c in enumerate(centroids_assigment):
    for z, v in enumerate(c):
      if v < centroids_assigment[assignments[z]][z]:
        assignments[z] = i
...
def updateCentroids(dataset, centroids, assignments):
  ...
  for i,c in enumerate(centroids):
    cluster = []
    for z,a in enumerate(assignments):
      if a == i:
        cluster.append(z)
    if len(cluster) > 0:
      centroids[i] = np.average(dataset[cluster], axis=0)
...
def calculateSSE(dataset, centroids, assignments):
  ...
  for i,c in enumerate(centroids):
    ...
    for z, a in enumerate(assignments):
      if a == i:
        indexs.append(z)
    cluster = dataset[indexs]
    sse += np.sum(np.linalg.norm(cluster - c, axis=1)**2)
  return sse
