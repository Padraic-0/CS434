...
def main():
  ...
  plt.title(f"Logistic Regression Training Curve (Step size: {step_size})")
  plt.xlabel("Epoch")
  ...
  #w, bias_losses = trainLogistic(X_train_bias,y_train,10000 ,0.0001)
  X_train_bias = dummyAugment(X_test)

  y_pred_train = X_train_bias@w >= 0

  ids = np.arange(len(y_pred_train))

  test_out = np.column_stack((ids, y_pred_train))
...
def logistic(z):
  ...
  #logit_z = np.where(z >= 0, 1 / (1 + np.exp(-z)), np.exp(z) / ( 1 + np.exp(z)))
  return logit_z

def log_logistic(z):
  #raise Exception('Student error: You haven\'t implemented the logistic calculation yet.')
  log_logit_z = np.where(z > 0, -np.logaddexp(0, -z), -np.logaddexp(z, 0) + z)
  return log_logit_z

def log_one_minus_logistic(z):
  log_one_minus_logit_z = np.where(z > 0, -np.logaddexp(0, z), -np.logaddexp(-z, 0) - z)
  return log_one_minus_logit_z
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  nll = -np.sum(y * log_logistic(X@w) + (1 - y) * log_one_minus_logistic(X@w))#np.log((1 - logistic(X@w))))
  return nll
...
def dummyAugment(X):
  ...
  aug_X = np.hstack((column_of_ones, X))
  return aug_X
