...
number_of_layers = 2
width_of_layers = 300  # only matters if number of layers > 1
...
def main():
  ...
  ids = np.arange(len(digits))

  test_out = np.column_stack((ids, digits))
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    self.grad_bias = np.sum(grad, axis=0)    # Compute dL / db as in Eq . 27
    return grad@self.weights.T               # Compute dL / dX as in Eq . 35
